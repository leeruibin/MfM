...
class MfMPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        depth_pipe: Optional[DepthEstimationPipeline],
        ...
        # )
        self.register_modules(
            tokenizer=tokenizer, text_encoder=text_encoder, vae=vae, depth_pipe=depth_pipe, 
            transformer=transformer, scheduler=scheduler,
        )
    ...
    def __call__(
        ...
        if input_path is not None:
            ...
            depth_cond = torch.zeros(batch_size, 1, num_frames, frames_tensor.shape[-2], frames_tensor.shape[-1]).to(device, prompt_embeds.dtype)
        ...
        if input_path is not None:
            ...
            images_cond = torch.zeros(batch_size, 4, num_frames, height, width).to(device, prompt_embeds.dtype)
        ...
        latents = latents[:,:self.transformer.config['out_channels']]

        if not output_type == "latent":
            vae_scaling_factor = 0.958917567
            # video = 1 / self.cfg.model.vae_scaling_factor * latents
            video = 1 / vae_scaling_factor * latents
            try:
